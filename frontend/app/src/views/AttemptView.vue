...
async function saveRankingDraft(question) {
  if (!question.answer || isReadOnly.value || showIntro.value) return false
  ...
  return withSaving(question.id, async () => {
    const updatedItems = await saveRanking(question, draftOrder)
    rankingItems.value = [
...
async function confirmRankingReset() {
  const question = rankingResetCandidate.value
  if (!question?.answer || isReadOnly.value || showIntro.value) {
    rankingResetCandidateId.value = null
    return
  }
...
async function toggleSkip(question) {
  ...
  await withSaving(question.id, async () => {
    ...
    if (rankingOrder(question).length) {
        ...
        <template v-for="(question, index) in normalizedQuestions" :key="question.id">
          ...
          <div v-else class="mt-5">
            ...
            <div v-else-if="question.question_type === 'ranking'" class="grid gap-3">
              <div v-for="(item, position) in rankingItemsForDisplay(question)" :key="item.option_id" class="flex items-center justify-between gap-4 rounded-[24px] border border-slate-200/80 bg-white/85 px-4 py-4">
